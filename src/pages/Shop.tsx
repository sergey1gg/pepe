...
const [tags, selectedTags] =useState<string[]>([hash])
  ...
  console.log(filteredData, tags)
    return (
        <main className=" bg-primary w-full h-full text-left text-xl text-black font-h-1">
            <Header searchInfo={featuresData.map(item => ({ id: item._id, name: item.name }))}/>
            {filter? (
            ...
            <div className="w-auto flex px-5 flex-col md:flex-row gap-3 mt-[32px] md:mt-[61px]">
                <div className='flex flex-row w-full gap-2'>
                    <input className="w-full rounded-21xl bg-surface overflow-hidden  py-3.5  pl-5 box-border relative gap-[16px] text-darkgray-300 font-body" placeholder='What are you looking for?'
            ...
            <div className='gap-3 flex justify-between flex-wrap px-5 mt-5'>
                ...
                {tags.map((tag, index) =>(
                    <div key={index} className="relative rounded-[10px] bg-white box-border w-auto flex flex-row py-1 lg:pr-6 pr-2 pl-2 items-center justify-start gap-[8px] text-left text-mid text-black font-links border-[1px] border-solid border-black">
                        <div className="rounded-[50px] bg-primary flex flex-row p-1 items-start justify-start"
                    ...
                    onClick={()=> {selectedTags([]); navigate("/shop")}}>
